...
# don't generate a key over and over again if we already have one
if !File.exist?(pkey)
  ...
  ruby_block 'node-save-pubkey' do
    block do
      node.set['jenkinsstack']['jenkins_slave_ssh_pubkey'] = File.read("#{pkey}.pub")
      node.save unless Chef::Config['solo']
    end
  end
end

# Create private key credentials (must do every time for slave LWRP to use)
